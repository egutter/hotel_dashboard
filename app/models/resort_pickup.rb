class ResortPickup
  ...
  def add_financial_transactions_for_start_date(daily_reservation)
    add_daily_reservation_for_key(daily_reservation, :start_date_financial_transactions)
  end

  def add_financial_transactions_for_end_date(daily_reservation)
    add_daily_reservation_for_key(daily_reservation, :end_date_financial_transactions)
  end
  ...
  def each_day_pickup
    @daily_reservations_by_date.each do |reservation_date, start_and_end_date_daily_reservations|
      ...
      start_date_daily_reservations_stats = ResortStats.new(@resort,
                                                            ...
                                                            start_and_end_date_daily_reservations[:start_date_financial_transactions])
      end_date_daily_reservations_stats = ResortStats.new(@resort,
                                                            reservation_date,
                                                            start_and_end_date_daily_reservations[:end_date_reservations],
                                                            start_and_end_date_daily_reservations[:end_date_financial_transactions])
  ...
  def add_daily_reservation_for_key(daily_reservation, key)
    @daily_reservations_by_date[daily_reservation.reservation_date] ||= {
      ...
      :start_date_financial_transactions => [],
      :end_date_financial_transactions => []
    }
